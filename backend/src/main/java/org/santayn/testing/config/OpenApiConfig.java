...
public class OpenApiConfig {
    ...
    private static String fallbackTag(String path) {
        ...
        if (path.startsWith("/api/v1/public/learning")) {
        ...
        if (path.startsWith("/api/v1/lectures")) {
            return "Лекции";
        }
        ...
        if (path.startsWith("/api/v1/faculties")) {
            return "Факультеты";
        }
        if (path.startsWith("/api/v1/groups")) {
            return "Группы";
        }
        if (path.startsWith("/api/v1/subjects")) {
            return "Предметы";
        }
        return "Система";
    }
    ...
    private static Map<OperationKey, OperationDoc> operationDocs() {
        ...
        put(docs, "GET", "/api/v1/status", "Система", "Проверить состояние API",
                "Liveness-проверка: подтверждает, что процесс Spring отвечает, но не проверяет PostgreSQL.");
        put(docs, "GET", "/api/v1/status/readiness", "Система", "Проверить готовность API",
                "Readiness-проверка: выполняет SELECT 1 и возвращает 200 только при доступной PostgreSQL.");

        put(docs, "POST", "/api/v1/auth/login", "Аутентификация", "Войти в систему",
        ...
        put(docs, "POST", "/api/v1/auth/register", "Аутентификация", "Зарегистрировать пользователя",
                "Создает новую учетную запись без привязки к существующей персоне и сразу возвращает JWT-токены. Привязку personId выполняет администратор отдельной защищенной операцией.");
        put(docs, "POST", "/api/v1/auth/refresh", "Аутентификация", "Обновить токены",
        ...
        put(docs, "POST", "/api/v1/auth/revoke", "Аутентификация", "Отозвать refresh-токен",
        ...
        put(docs, "POST", "/api/v1/auth/change-password", "Аутентификация", "Сменить пароль",
        ...
        put(docs, "GET", "/api/v1/auth/me", "Аутентификация", "Получить текущего пользователя",
                "Возвращает сведения о текущей авторизованной учетной записи: логин, personId, роли, permissions и связанные данные профиля.");

        put(docs, "GET", "/api/v1/faculties", "Факультеты", "Получить список факультетов",
        ...
        put(docs, "GET", "/api/v1/faculties/{id}", "Факультеты", "Получить факультет по ID",
        ...
        put(docs, "POST", "/api/v1/faculties", "Факультеты", "Создать факультет",
        ...
        put(docs, "PUT", "/api/v1/faculties/{id}", "Факультеты", "Обновить факультет",
                "Изменяет название, код и описание существующего факультета. Идентификатор берется из пути, а новые значения из тела запроса.");
        put(docs, "DELETE", "/api/v1/faculties/{id}", "Факультеты", "Удалить факультет",
        ...
        put(docs, "GET", "/api/v1/subjects", "Предметы", "Получить список предметов",
        ...
        put(docs, "GET", "/api/v1/subjects/{id}", "Предметы", "Получить предмет по ID",
                "Возвращает название и описание конкретного предмета по его идентификатору.");
        put(docs, "POST", "/api/v1/subjects", "Предметы", "Создать предмет",
        ...
        put(docs, "PUT", "/api/v1/subjects/{id}", "Предметы", "Обновить предмет",
                "Изменяет название и описание существующего предмета. Идентификатор предмета берется из URL.");
        put(docs, "DELETE", "/api/v1/subjects/{id}", "Предметы", "Удалить предмет",
        ...
        put(docs, "GET", "/api/v1/users", "Пользователи", "Получить список пользователей",
                "Возвращает учетные записи пользователей вместе с ролями, permissions, активностью и связанной персоной, если она назначена.");
        put(docs, "GET", "/api/v1/users/{id}", "Пользователи", "Получить пользователя по ID",
                "Возвращает подробную карточку пользователя по идентификатору учетной записи.");
        put(docs, "GET", "/api/v1/users/me", "Пользователи", "Получить свой профиль",
        ...
        put(docs, "GET", "/api/v1/users/people", "Пользователи", "Получить список персон",
        ...
        put(docs, "GET", "/api/v1/users/people/{personId}", "Пользователи", "Получить персону по ID",
        ...
        put(docs, "POST", "/api/v1/users/people", "Пользователи", "Создать персону",
                "Создает запись физического лица с именем, фамилией, email, датой рождения и телефоном. После создания администратор может привязать personId к учетной записи и использовать его в memberships.");
        put(docs, "PUT", "/api/v1/users/people/{personId}", "Пользователи", "Обновить персону",
        ...
        put(docs, "PUT", "/api/v1/users/{id}/active", "Пользователи", "Изменить активность пользователя",
        ...
        put(docs, "PUT", "/api/v1/users/{id}/person", "Пользователи", "Привязать персону к пользователю",
                "Администратор привязывает существующий personId к учетной записи. Значение null снимает текущую привязку; одна персона не может принадлежать двум пользователям.");
        put(docs, "PUT", "/api/v1/users/{id}/roles", "Пользователи", "Назначить роли пользователю",
        ...
        put(docs, "PUT", "/api/v1/users/{id}/permissions", "Пользователи", "Назначить индивидуальные права",
        ...
        put(docs, "GET", "/api/v1/roles", "Роли и права", "Получить список ролей",
        ...
        put(docs, "PUT", "/api/v1/roles/{id}/permissions", "Роли и права", "Настроить права роли",
        ...
        put(docs, "GET", "/api/v1/courses/templates", "Курсы", "Получить шаблоны курсов",
        ...
        put(docs, "GET", "/api/v1/courses/templates/{templateId}", "Курсы", "Получить шаблон курса",
                "Возвращает один шаблон курса по идентификатору: предмет, автор, название и признак публичной видимости.");
        put(docs, "POST", "/api/v1/courses/templates", "Курсы", "Создать шаблон курса",
                "Создает шаблон курса для выбранного предмета и автора. Версии курса создаются отдельно внутри этого шаблона.");
        put(docs, "PUT", "/api/v1/courses/templates/{templateId}", "Курсы", "Обновить шаблон курса",
                "Изменяет предмет, автора, название и публичность существующего шаблона курса.");
        put(docs, "DELETE", "/api/v1/courses/templates/{templateId}", "Курсы", "Удалить шаблон курса",
        ...
        put(docs, "PUT", "/api/v1/courses/versions/{versionId}/publish", "Курсы", "Опубликовать версию курса",
        ...
        put(docs, "PUT", "/api/v1/courses/versions/{versionId}/unpublish", "Курсы", "Снять публикацию версии курса",
                "Делает версию курса недоступной в публичных учебных сценариях. Сама версия и ее лекции остаются в базе для редактирования.");
        put(docs, "GET", "/api/v1/courses/templates/{templateId}/versions", "Курсы", "Получить версии шаблона",
        ...
        put(docs, "GET", "/api/v1/courses/versions/{versionId}", "Курсы", "Получить версию курса",
                "Возвращает конкретную версию курса: номер версии, заголовок, описание, автора, статус публикации и заметки об изменениях.");
        put(docs, "POST", "/api/v1/courses/templates/{templateId}/versions", "Курсы", "Создать версию курса",
        ...
        put(docs, "PUT", "/api/v1/courses/versions/{versionId}", "Курсы", "Обновить версию курса",
                "Изменяет номер, заголовок, описание, автора и заметки существующей версии курса без отдельного изменения статуса публикации.");

        put(docs, "GET", "/api/v1/lectures", "Лекции", "Получить список лекций",
        ...
        put(docs, "GET", "/api/v1/lectures/{id}", "Лекции", "Получить лекцию по ID",
                "Возвращает данные лекции: версию курса, порядковый номер, заголовок, описание, ключ папки контента и публичность.");
        put(docs, "POST", "/api/v1/lectures", "Лекции", "Создать лекцию",
        ...
        put(docs, "GET", "/api/v1/memberships/faculties/{facultyId}", "Участники", "Получить участников факультета",
                "Возвращает активных участников конкретного факультета. Используется на странице управления составом факультета.");
        put(docs, "GET", "/api/v1/memberships/faculties/memberships/{membershipId}", "Участники", "Получить membership факультета",
                "Возвращает одну membership-запись факультета по идентификатору, включая personId, роль, статус и заметки.");
        put(docs, "POST", "/api/v1/memberships/faculties/{facultyId}", "Участники", "Добавить участника факультета",
        ...
        put(docs, "PUT", "/api/v1/memberships/faculties/memberships/{membershipId}/status", "Участники", "Изменить статус участника факультета",
        ...
        put(docs, "PUT", "/api/v1/memberships/faculties/memberships/{membershipId}", "Участники", "Обновить membership факультета",
                "Изменяет статус и заметки membership-записи факультета без смены привязанного человека и факультета.");
        put(docs, "GET", "/api/v1/memberships/groups", "Участники", "Найти участников групп",
        ...
        put(docs, "POST", "/api/v1/memberships/groups/{groupId}", "Участники", "Добавить участника группы",
        ...
        put(docs, "GET", "/api/v1/memberships/groups/{groupId}", "Участники", "Получить участников группы",
        ...
        put(docs, "GET", "/api/v1/memberships/groups/memberships/{membershipId}", "Участники", "Получить membership группы",
                "Возвращает одну membership-запись группы по идентификатору, включая personId, роль, статус и заметки.");
        put(docs, "PUT", "/api/v1/memberships/groups/memberships/{membershipId}/status", "Участники", "Изменить статус участника группы",
        ...
        put(docs, "PUT", "/api/v1/memberships/groups/memberships/{membershipId}", "Участники", "Обновить membership группы",
                "Изменяет статус и заметки membership-записи группы без изменения связей groupId и personId.");
        put(docs, "GET", "/api/v1/memberships/subjects", "Участники", "Найти участников предметов",
        ...
        put(docs, "GET", "/api/v1/memberships/subjects/{subjectId}", "Участники", "Получить участников предмета",
                "Возвращает активных участников выбранного предмета. Эти записи участвуют в настройке учебной нагрузки.");
        put(docs, "GET", "/api/v1/memberships/subjects/memberships/{membershipId}", "Участники", "Получить membership предмета",
                "Возвращает одну membership-запись предмета по идентификатору.");
        put(docs, "POST", "/api/v1/memberships/subjects/{subjectId}", "Участники", "Добавить участника предмета",
        ...
        put(docs, "PUT", "/api/v1/memberships/subjects/memberships/{membershipId}/status", "Участники", "Изменить статус участника предмета",
                "Обновляет только статус membership-записи предмета. Подходит для активации и деактивации преподавателя по предмету.");
        put(docs, "PUT", "/api/v1/memberships/subjects/memberships/{membershipId}", "Участники", "Обновить membership предмета",
                "Изменяет статус и заметки membership-записи предмета без смены предмета и человека.");

        put(docs, "GET", "/api/v1/teaching/load-types", "Учебная нагрузка", "Получить типы учебной нагрузки",
        ...
        put(docs, "GET", "/api/v1/teaching/load-types/{loadTypeId}", "Учебная нагрузка", "Получить тип нагрузки",
                "Возвращает один тип учебной нагрузки по идентификатору.");
        put(docs, "POST", "/api/v1/teaching/load-types", "Учебная нагрузка", "Создать тип нагрузки",
                "Создает новый тип учебной нагрузки с названием и описанием. После создания его можно назначать subject membership-записям.");
        put(docs, "PUT", "/api/v1/teaching/load-types/{loadTypeId}", "Учебная нагрузка", "Обновить тип нагрузки",
                "Изменяет название и описание существующего типа учебной нагрузки.");
        put(docs, "GET", "/api/v1/teaching/subject-load-types", "Учебная нагрузка", "Найти типы нагрузки по предметам",
        ...
        put(docs, "POST", "/api/v1/teaching/subject-memberships/{subjectMembershipId}/load-types", "Учебная нагрузка", "Добавить тип нагрузки преподавателю",
        ...
        put(docs, "PUT", "/api/v1/teaching/subject-load-types/{subjectLoadTypeId}/status", "Учебная нагрузка", "Изменить статус типа нагрузки по предмету",
        ...
        put(docs, "PUT", "/api/v1/teaching/subject-load-types/{subjectLoadTypeId}", "Учебная нагрузка", "Обновить тип нагрузки по предмету",
                "Изменяет статус и заметки связи subjectMembershipId с teachingLoadTypeId.");
        put(docs, "GET", "/api/v1/teaching/assignments", "Учебная нагрузка", "Найти преподавательские назначения",
        ...
        put(docs, "GET", "/api/v1/teaching/assignments/{id}", "Учебная нагрузка", "Получить преподавательское назначение",
                "Возвращает одно преподавательское назначение по идентификатору, включая группу, курс, семестр, учебный год, часы и статус.");
        put(docs, "POST", "/api/v1/teaching/assignments", "Учебная нагрузка", "Создать преподавательское назначение",
        ...
        put(docs, "PUT", "/api/v1/teaching/assignments/{assignmentId}", "Учебная нагрузка", "Обновить преподавательское назначение",
                "Изменяет состав, курс, период, часы, статус и заметки существующего преподавательского назначения.");
        put(docs, "PUT", "/api/v1/teaching/assignments/{assignmentId}/status", "Учебная нагрузка", "Изменить статус назначения",
        ...
        put(docs, "GET", "/api/v1/teaching/enrollments", "Учебная нагрузка", "Найти enrollments студентов",
        ...
        put(docs, "POST", "/api/v1/teaching/assignments/{assignmentId}/enrollments", "Учебная нагрузка", "Добавить enrollment студента",
        ...
        put(docs, "PUT", "/api/v1/teaching/enrollments/{enrollmentId}/status", "Учебная нагрузка", "Изменить статус enrollment",
        ...
        put(docs, "GET", "/api/v1/teaching/lecture-assignments", "Учебная нагрузка", "Найти назначения лекций",
                "Возвращает связи лекций с преподавательскими назначениями. Можно фильтровать по teachingAssignmentId и courseLectureId.");
        put(docs, "POST", "/api/v1/teaching/assignments/{assignmentId}/lecture-assignments", "Учебная нагрузка", "Назначить лекцию группе",
        ...
        put(docs, "PUT", "/api/v1/teaching/lecture-assignments/{lectureAssignmentId}", "Учебная нагрузка", "Обновить назначение лекции",
                "Изменяет лекцию, сроки, обязательность, минимальный процент прогресса и статус существующего назначения лекции.");
        put(docs, "PUT", "/api/v1/teaching/lecture-assignments/{lectureAssignmentId}/status", "Учебная нагрузка", "Изменить статус назначения лекции",
                "Обновляет только статус назначения лекции. Используйте для публикации, закрытия или отключения лекции для студентов.");
        put(docs, "GET", "/api/v1/teaching/lecture-progress", "Учебная нагрузка", "Получить прогресс по лекциям",
        ...
        put(docs, "POST", "/api/v1/teaching/lecture-assignments/{lectureAssignmentId}/progress", "Учебная нагрузка", "Обновить прогресс студента",
        ...
        put(docs, "PUT", "/api/v1/tests/{id}", "Тесты", "Обновить тест",
                "Изменяет только название, описание, длительность, лимит попыток и количество вопросов. Правила выбора изменяются исключительно отдельной операцией selection-rules.");
        put(docs, "GET", "/api/v1/tests/{testId}/selection-rules", "Тесты", "Получить правила выбора вопросов",
                "Возвращает правила, по которым тест набирает вопросы из лекций: общее число, текстовые, одиночные и множественные варианты.");
        put(docs, "PUT", "/api/v1/tests/{testId}/selection-rules", "Тесты", "Заменить правила выбора вопросов",
        ...
        put(docs, "GET", "/api/v1/tests/assignments", "Тесты", "Найти назначения тестов",
        ...
        put(docs, "GET", "/api/v1/tests/assignments/{assignmentId}", "Тесты", "Получить назначение теста",
                "Возвращает одно назначение теста по идентификатору, включая область назначения, сроки доступности и статус.");
        put(docs, "POST", "/api/v1/tests/{testId}/assignments", "Тесты", "Назначить тест",
        ...
        put(docs, "PUT", "/api/v1/tests/assignments/{assignmentId}", "Тесты", "Обновить назначение теста",
                "Изменяет область, сроки и статус существующего назначения теста.");
        put(docs, "PUT", "/api/v1/tests/assignments/{assignmentId}/status", "Тесты", "Изменить статус назначения теста",
                "Обновляет только статус назначения теста. Позволяет публиковать, закрывать или отключать доступ к тесту.");
        put(docs, "GET", "/api/v1/tests/attempts", "Тесты", "Найти попытки тестов",
                "Возвращает попытки прохождения с фильтрами по testAssignmentId и personId. Используется для просмотра истории прохождений.");
        put(docs, "GET", "/api/v1/tests/attempts/{attemptId}", "Тесты", "Получить попытку теста",
                "Возвращает одну попытку теста по идентификатору: студент, назначение, статус, баллы и временные метки.");
        put(docs, "POST", "/api/v1/tests/assignments/{assignmentId}/attempts", "Тесты", "Начать попытку теста",
        ...
        put(docs, "GET", "/api/v1/tests/attempts/{attemptId}/responses", "Тесты", "Получить ответы попытки",
                "Возвращает все ответы, сохраненные в рамках конкретной попытки теста.");
        put(docs, "POST", "/api/v1/tests/attempts/{attemptId}/responses", "Тесты", "Сохранить ответ на вопрос",
                "Сохраняет текстовый ответ или выбранные варианты по одному вопросу. Правильность и начисленные баллы вычисляет только сервер.");
        put(docs, "GET", "/api/v1/tests/responses/{responseId}/selected-options", "Тесты", "Получить выбранные варианты ответа",
        ...
        put(docs, "POST", "/api/v1/tests/attempts/{attemptId}/complete", "Тесты", "Завершить попытку теста",
                "Закрывает активную попытку; итоговый балл и статус вычисляет сервер по сохраненным ответам. Повторное завершение отклоняется.");

        put(docs, "GET", "/api/v1/questions", "Вопросы", "Получить вопросы теста",
        ...
        put(docs, "GET", "/api/v1/questions/{questionId}", "Вопросы", "Получить вопрос по ID",
                "Возвращает один вопрос теста: тип, текст, баллы, порядок, правильный текстовый ответ, активность и привязку к лекции.");
        put(docs, "GET", "/api/v1/questions/{questionId}/options", "Вопросы", "Получить варианты вопроса",
        ...
        put(docs, "POST", "/api/v1/questions/import", "Вопросы", "Импортировать вопросы из DOCX",
        ...
        put(docs, "POST", "/api/v1/questions", "Вопросы", "Создать вопрос",
        ...
        put(docs, "PUT", "/api/v1/questions/{questionId}", "Вопросы", "Обновить вопрос",
                "Изменяет тип, текст, баллы, порядок, правильный ответ, активность и привязку вопроса к лекции.");
        put(docs, "GET", "/api/v1/questions/options/{optionId}", "Вопросы", "Получить вариант ответа",
                "Возвращает один вариант ответа по идентификатору, включая текст, порядок и признак правильности.");
        put(docs, "POST", "/api/v1/questions/{questionId}/options", "Вопросы", "Добавить вариант ответа",
        ...
        put(docs, "PUT", "/api/v1/questions/options/{optionId}", "Вопросы", "Обновить вариант ответа",
                "Изменяет текст, порядок и признак правильности существующего варианта ответа.");
        put(docs, "PUT", "/api/v1/questions/{questionId}/active", "Вопросы", "Изменить активность вопроса",
                "Включает или отключает вопрос. Неактивные вопросы не должны попадать в случайную выборку при старте попытки.");

        put(docs, "GET", "/api/v1/public/learning/subjects/{subjectId}", "Публичное обучение", "Получить публичный предмет",
                "Возвращает краткую информацию о предмете для учебного frontend-сценария: идентификатор, название и описание.");
        put(docs, "GET", "/api/v1/public/learning/subjects/{subjectId}/lectures", "Публичное обучение", "Получить публичные лекции предмета",
                "Возвращает опубликованные и публично видимые лекции по всем опубликованным версиям курсов выбранного предмета.");
        put(docs, "GET", "/api/v1/public/learning/lectures/{lectureId}", "Публичное обучение", "Получить публичную лекцию",
                "Возвращает данные лекции только если лекция, версия курса и шаблон курса публично доступны и опубликованы.");
        put(docs, "GET", "/api/v1/public/learning/lectures/{lectureId}/tests", "Публичное обучение", "Получить доступные тесты лекции",
                "Возвращает активные тесты, назначенные на лекцию и доступные в текущий момент времени.");
        put(docs, "GET", "/api/v1/public/learning/tests/{testId}", "Публичное обучение", "Получить метаданные теста",
                "Без побочных эффектов возвращает карточку теста, attemptsRemaining и canResume. Не создает и не возобновляет попытку.");
        put(docs, "POST", "/api/v1/public/learning/test-assignments/{assignmentId}/attempts/start", "Публичное обучение", "Начать или продолжить попытку",
                "Единственная операция старта: создает новую попытку или возвращает незавершенную попытку текущего пользователя. Вопросы выбираются случайно с учетом правил теста.");
        put(docs, "POST", "/api/v1/public/learning/attempts/{attemptId}/submit", "Публичное обучение", "Отправить ответы попытки",
        ...
        put(docs, "GET", "/api/v1/results/teacher/subjects", "Результаты", "Получить предметы преподавателя",
        ...
        put(docs, "GET", "/api/v1/results/teacher/lectures", "Результаты", "Получить лекции для отчета",
        ...
        put(docs, "GET", "/api/v1/results/teacher/tests", "Результаты", "Получить тесты по лекции",
                "Возвращает тесты, назначенные на lectureId. Список используется для фильтрации результатов по конкретному тесту.");
        put(docs, "GET", "/api/v1/results/teacher/groups", "Результаты", "Получить группы по тесту",
                "Возвращает группы, которые связаны с выбранным testId через назначения, учебную нагрузку или фактические попытки студентов.");
        put(docs, "GET", "/api/v1/results/teacher/students", "Результаты", "Получить студентов группы",
        ...
        put(docs, "GET", "/api/v1/results/teacher/data", "Результаты", "Получить данные отчета",
